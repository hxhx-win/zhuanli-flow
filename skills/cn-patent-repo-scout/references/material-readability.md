...
> - `cn-patent-project-drafting`：步骤 0.1 的 `patent-env-check.py --source-root` 使用本表的格式判定与工具命令（脚本内 `EXT_READABILITY` / `EXT_TOOLS` / `INSTALL_COMMANDS` 与本表保持同步，本表为权威来源）。修改时两边一起改。
