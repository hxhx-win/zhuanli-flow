...
主跑型 skill，在 `step-3.inventor-review` 阶段由 `cn-patent-domain-runtime` 编排器加载，引导用户对交底初稿做 3 阶段审核。
...
**REQUIRED CONTEXT：** 状态字段约定见 `cn-patent-domain-runtime/references/state-machine-reference.md`。
