...
- 宿主 Agent 负责专业结果回写、用户交互（Claude Code 用 AskUserQuestion；Cursor/Codex 等价）、脚本调用、Gate 拍板、verdict 分支和风险登记同步；流程控制字段必须通过 Domain Runtime 提交
- subagent 直接 Write 落盘到用户项目根目录；写到磁盘的内容**不返回**给宿主 Agent
- subagent 返回宿主 Agent 的摘要严格 ≤ 600 字（step 1/2/3 评审/3 交底书）或按子 skill SKILL.md 字数上限
- 宿主 Agent 派 subagent 前必须把已确认事实（用户决策、verdict、关键技术口径校正、上游产物路径）**显式写进 prompt**，subagent 不得自行猜测
- 直读直写大体量产物属于跳步反模式，见 `orchestration-philosophy.md § 跳步红旗`

例外（宿主 Agent 直接做，不派 subagent）：

- 专业 Skill 业务结果字段的回写；`current_stage` 等流程控制字段只能通过 Runtime 写入
- 脚本调用与 stderr/stdout 回显
- 用户交互、Gate 确认包条目展示
- 产物存在性与行数核对（Bash `ls` / `wc -l`）
- 用户已明确单点修订（如只让改 1~2 处措辞），可宿主 Agent直接 Edit
...
| Runtime 状态/校验/迁移 | domain-runtime.py | 宿主 Agent 调用，统一输出固定 JSON 信封 |
| Stage 前置/出口断言（兼容） | validate-stage.py | 保留既有输出契约 |
| 下一步路由（兼容） | get-next-step.py | 保留既有输出契约 |
...
| 阶段 | 执行者 | 工具 |
|---|---|---|
| step 0 环境预检 + 状态初始化 | 宿主 Agent | Bash 调 `patent-env-check.py` / `new-iteration-state.py` |
| step 0.3 资料角色确认 | 宿主 Agent | AskUserQuestion + 回写资料角色业务字段 |
...
| step 3 用户分流决策 | 宿主 Agent | AskUserQuestion + Runtime transition |
| Gate A 起草前决策 | 宿主 Agent | AskUserQuestion + Read (decision-categories.json + state 文件) + Runtime transition |
| Gate A 确认 | 宿主 Agent | AskUserQuestion + Runtime transition |
| handoff 段(packaged 切换) | 宿主 Agent | Bash 调 `scripts/handoff-package.py` |
| handoff 段(picked_up 切换) | 宿主 Agent | Bash 调 `scripts/handoff-pickup.py` |
| handoff 段(notes 准备状态) | 宿主 Agent | AskUserQuestion + Bash 验证文件存在 + Runtime transition |
| handoff 段(local 切换) | 宿主 Agent | Runtime transition |
...
| step 5 审稿前风险确认 | 宿主 Agent | AskUserQuestion + Runtime transition |
| **step 6 代理师审稿（前置：模式选择）** | **宿主 Agent AskUserQuestion 选 single/multi，并通过 Runtime 写入 `state.step_6.review_mode` + `state.step_6.review_mode_selected_at`（伪产物 `reviews/review-mode-selection.md` 已废）** | **AskUserQuestion + Runtime transition** |
...
| step 7 Gate B 用户确认 | 宿主 Agent | AskUserQuestion + Runtime transition |
| step 8 反馈修订 | **`cn-patent-attorney-review` 派 revision subagent (mode=revision)** | **Agent (subagent_type=general-purpose)** |
| step 9 Gate C 质量检查 + 交付 | 宿主 Agent | Bash 调 `automated_quality_check.py` + AskUserQuestion + Runtime transition |
...
字段口径以 `cn-patent-disclosure-draft/SKILL.md` 的「subagent 入口分路」与「宿主 Agent派 subagent prompt 契约」两段为权威；本表与 SKILL.md 任一处修改必须同步另一处。
