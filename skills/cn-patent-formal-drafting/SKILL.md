...
- [ ] `patent/<patent-slug>/state/drafting-context.json`（编排器在 step 4 派单前用 `cn-patent-domain-runtime/scripts/extract-drafting-context.py` 生成；含 `gate_a`、`handoff`、`pre_review` 三个子树，~3 KB；起草 subagent **不再 Read state.json 全量**）
