...
`needs_supplement` 不触发硬阻断,仅在 Gate A 段入口由编排器输出 warning + 候选补料项。详细见 `cn-patent-domain-runtime/references/orchestration-philosophy.md § Gate 哲学` 与 `scripts/lib/preconditions.py` 中 `gate-a.drafting-decisions` enter 条件。
